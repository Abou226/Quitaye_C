namespace Controllers
{
    ...
    [ApiController]
    [Authorize]
    public class ClientPromotionsController : GenericController<Promotion, Client, List<PromotionOffre>, PromotionType>
    {
        private readonly IGenericRepositoryWrapper<Promotion, Client, List<PromotionOffre>, PromotionType> repositoryWrapper;
        private readonly IConfigSettings _settings;
        private readonly IMapper _mapper;
        private readonly IGenericRepositoryWrapper<EntrepriseUser> _entrepriseUser;

        public ClientPromotionsController(IGenericRepositoryWrapper<Promotion, Client, 
        ...
        public async Task<ActionResult<IEnumerable<Promotion>>> GetBy([FromRoute] Guid id)
        {
            try
            {
                ...
                if (identity.Count() != 0)
                {
                    var result = await repositoryWrapper.Item.GetByInclude(x => (x.EntrepriseId == id), x => x.OffreList, x => x.Type);
                    return Ok(result);
                }
                else return NotFound("User not indentified");
            }
