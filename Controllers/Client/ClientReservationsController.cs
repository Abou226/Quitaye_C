...
namespace Controllers
{
    ...
    public class ClientReservationsController : GenericController<Reservation, Client, Gamme, Marque, Taille, Model, Categorie, Style, Marque>
    {
        ...
        public override async Task<ActionResult<Reservation>> PatchUpdateAsync([FromForm] JsonPatchDocument value, [FromHeader] Guid id)
        {
            try
            {
        ...
        public override async Task<ActionResult<IEnumerable<Reservation>>> GetBy(string search)
        {
            try
            {
                ...
                {
                    var result = await repositoryWrapper.Item.GetByInclude(x =>
                                (x.ClientId == identity.FirstOrDefault().Id)
