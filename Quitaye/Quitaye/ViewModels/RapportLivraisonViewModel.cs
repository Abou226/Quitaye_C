...
namespace Quitaye
{
    public class RapportLivraisonViewModel : BaseVM.BaseViewModel
    {
            ...
            if (!string.IsNullOrWhiteSpace(search))
            {
                if (Items.Count() != 0)
                {
                    ...
                    //if (items.Count() != 0)
                    {
                        ...
                        {
                            var fsd = from d in Items where d.Date_Livraison.Date == item.Key.Date orderby d.DateOfCreation select d;
        ...
        public RapportLivraisonViewModel(INavigation navigation, Entreprise entreprise) : this()
        {
