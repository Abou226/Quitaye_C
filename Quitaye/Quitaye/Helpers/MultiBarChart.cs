...
namespace Quitaye.Helpers
{
    public class MultiBarChart : BarChart
    {
        ...
        private void Init()
        {
            ...
            foreach (var item in MultiBarEntries)
            {

                foreach (var l in item)
                {
                    if(l.Value >= multibarMax)
                    {
                        multibarMax = l.Value;
                    }

                    if (l.Value <= multibarMin)
                        multibarMin = l.Value;
                }
            }
        }
        ...
        private SKPoint[] CalculateMultilinePoints(SKSize itemSize, float origin, float headerHeight, int i)
        {
            ...
            for (int j = 0; j < this.Entries.Count(); j++)
            {
                ...
                var y = headerHeight + (((multibarMax - entry.Value) / (multibarMax - multibarMin)) * itemSize.Height);
        ...
        private void DrawLegend(SKCanvas canvas, int width, int height)
        {
            ...
            using (var paint = new SKPaint())
            {
                ...
                canvas.DrawText(NumbersTools.GetNember(multibarMax), 0, minPoint - 20, paint);
                canvas.DrawCircle(12, maxPoint, 5, paint);
                canvas.DrawText(NumbersTools.GetNember(multibarMin), 0, maxPoint - 20, paint);

                var step = maxPoint / 4;
                var valueStep = multibarMax / 4;
                for (int i = 1; i < 4; i++)
                {
