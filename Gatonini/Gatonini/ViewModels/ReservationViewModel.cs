        ...
        private async Task GetVentes(DateTime start, DateTime end)
        {
            if (BaseVM.IsInternetOn)
            {
                ...
                try
                {
                    ...
                    var Vente = await DataService.GetItemsAsync(await SecureStorage.GetAsync("Token"), "Ventes/" + start.ToString("MM-dd-yyyy") + "/" + end.ToString("MM-dd-yyyy"));
                    Items.Clear();
                    Total = 0;
                    ...
                    if (Vente.Count() != 0)
                    {
                        ...
                        {
                            Total += item.Prix_Vente_Unité;
                            item.Designation = item.Offre.Gamme.Marque.Name + "_" + item.Offre.Gamme.Style.Name;
