...
namespace Gatonini
{
    public class RapportPayementViewModel : BaseVM.BaseViewModel
    {
        ...
        public IDataService<Payement> DataService { get; }
        ...
        public RapportPayementViewModel()
        {
            ...
            DataService = DependencyService.Get<IDataService<Payement>>();
        ...
        private async Task GetItemsAsync()
        {
            if (BaseVM.IsInternetOn)
            {
                ...
                {
                    IsNotBusy = false;
                    var items = await DataService.GetItemsAsync(await SecureStorage.GetAsync("Token"), "Payements/" + Start.ToString("MM-dd-yyyy") + "/" + End.ToString("MM-dd-yyyy"));
                    Quantité = items.Count();
                    if (items.Count() != 0)
                    {
                        Items.Clear();
